let view = {
    ...
    /* Методы получают данные страницы о
    месте объекта и присваевают ему
...
let model = {
    ...
    generateShip: function() {
        ...
        if (direction === 1) {
            //Генерирует начальную позицию для горизонтального корабля
            row = Math.floor(Math.random() * this.boardSize);
            col = Math.floor(Math.random() * (this.boardSize - this.shipLength));
        } else {
            //Генерирует начальную позицию для вертикального корабля
...
let controller = {
    ...
    processGuess: function(guess) {
        ...
        if (location) {
            ...
            console.log(hit, model.shipsSunk, model.numShips)
